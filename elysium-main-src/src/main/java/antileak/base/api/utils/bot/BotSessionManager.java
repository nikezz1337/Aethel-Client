package antileak.base.api.utils.bot;
...
import antileak.base.client.modules.impl.player.AutoForest;
...
public final class BotSessionManager {
    private static final double BOT_ATTACK_RANGE = 4.5D;
    private static final long BOT_CONNECT_RESOURCE_PACK_GRACE_MS = 10_000L;
    ...
    public static void finishBotConnectStage() {
        ...
        bypassResourcePacksUntilMs = System.currentTimeMillis() + BOT_CONNECT_RESOURCE_PACK_GRACE_MS;
        ...
        MinecraftClient.getInstance().execute(() -> completePendingBotConnect(pending));
    }
    ...
    public static void connect(String name, String address) {
        ...
        bypassResourcePacksUntilMs = System.currentTimeMillis() + 60_000L;
    ...
    private static void completePendingBotConnect(PendingBotConnect pending) {
        ...
        try {
            if (mc.getNetworkHandler() != null && mc.world != null && mc.player != null) {
                connectedBot = freezeCurrentSession();
            }

            if (pending.previous() != null && activateSession(pending.previous())) {
                connections.remove(pending.previous());
                pendingBotConnect = null;
                return;
            }

            if (connectedBot != null && activateSession(connectedBot)) {
                connections.remove(connectedBot);
                pendingBotConnect = null;
                return;
            }

            restoreAfterConnectFailure(mc, pending.previous(), pending.originalSession(), pending.originalServerInfo());
        } finally {
            pendingBotConnect = null;
        }
    }
    ...
    private static void attackBotTarget(BotConnection bot) {
        Entity target = findBotAttackTarget(bot);
        if (target != null) {
            if (bot.interactionManager() != null) {
                bot.interactionManager().attackEntity(bot.player(), target);
            } else {
                bot.handler().sendPacket(PlayerInteractEntityC2SPacket.attack(target, false));
            }
        }
    ...
    private static Entity findBotAttackTarget(BotConnection bot) {
        ...
        Vec3d reachVec = eyePos.add(lookVec.multiply(BOT_ATTACK_RANGE));
        EntityHitResult result = ProjectileUtil.raycast(
                bot.player(),
                eyePos,
                reachVec,
                bot.player().getBoundingBox().expand(BOT_ATTACK_RANGE),
                entity -> entity != bot.player() && entity.isAlive() && !entity.isRemoved(),
                BOT_ATTACK_RANGE * BOT_ATTACK_RANGE
        );
