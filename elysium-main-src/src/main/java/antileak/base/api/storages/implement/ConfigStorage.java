...
import antileak.base.api.utils.cmd.macro.Macro;
import antileak.base.api.utils.draggable.Draggable;
import antileak.base.api.utils.namespaced.FileUtils;
import antileak.base.client.modules.Module;
import antileak.base.client.modules.impl.render.Interface;
import antileak.base.client.modules.impl.render.base.InterfaceProcessing;
import antileak.base.client.modules.impl.render.base.implement.TargetHud;
...
import antileak.base.client.modules.settings.implement.*;
...
public class ConfigStorage {
    ...
    private final String extension = ".elysium";
    ...
    public void saveConfig(String config) throws Exception {
        File file = new File(elysium.INSTANCE.configsDir, config + extension);

        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(config));
        object.add("theme", new JsonPrimitive(elysium.INSTANCE.themeStorage.getThemes().name()));
        object.add("language", new JsonPrimitive(elysium.INSTANCE.localizationStorage.getLanguage().name()));
    ...
    public void loadConfig(String config) throws Exception {
        if (!FileUtils.exists(elysium.INSTANCE.configsDir + "/" + config + extension)) return;
        JsonObject object;
        try (InputStream stream = Files.newInputStream(Paths.get(elysium.INSTANCE.configsDir + "/" + config + extension));
        ...
        if (object.has("theme")) {
            ...
            for (ThemeStorage.Themes theme : ThemeStorage.Themes.values()) {
                if (theme.name().equals(themeName)) {
                    elysium.INSTANCE.themeStorage.setThemes(theme);
                    break;
                }
            }
        }
        ...
        if (object.has("language")) {
            try {
                elysium.INSTANCE.localizationStorage.setLanguage(LocalizationStorage.Language.valueOf(object.get("language").getAsString()));
    ...
    public void saveGlobals() throws Exception {
        File file = new File(elysium.INSTANCE.globalsDir, "globals" + extension);
        JsonObject object = new JsonObject();
        ...
        elysium.INSTANCE.friendStorage.getFriends().forEach(friendsArray::add);
        object.add("friends", friendsArray);

        JsonArray staffsArray = new JsonArray();
        elysium.INSTANCE.staffStorage.getStaffs().forEach(staffsArray::add);
        object.add("staffs", staffsArray);

        JsonArray macrosArray = new JsonArray();
        elysium.INSTANCE.macroStorage.getMacros().forEach(macro -> {
    ...
    public void loadGlobals() throws Exception {
        if (!FileUtils.exists(elysium.INSTANCE.globalsDir + "/" + "globals" + extension)) return;
        JsonObject object;
        try (InputStream stream = Files.newInputStream(Paths.get(elysium.INSTANCE.globalsDir + "/" + "globals" + extension));
             Reader reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
            object = JsonParser.parseReader(reader).getAsJsonObject();
        }
        ...
        if (object.has("friends")) {
            for (JsonElement element : object.get("friends").getAsJsonArray()) {
                if (elysium.INSTANCE.friendStorage.isFriend(element.getAsString())) continue;
                elysium.INSTANCE.friendStorage.add(element.getAsString());
            }
        }

        if (object.has("staffs")) {
            for (JsonElement element : object.get("staffs").getAsJsonArray()) {
                if (elysium.INSTANCE.staffStorage.isStaff(element.getAsString())) continue;
                elysium.INSTANCE.staffStorage.add(element.getAsString());
            }
        }
        ...
        if (object.has("macros")) {
            for (JsonElement element : object.get("macros").getAsJsonArray()) {
                try {
                    ...
                    if (name.isBlank() || elysium.INSTANCE.macroStorage.getMacro(name) != null) {
                        continue;
                    }

                    elysium.INSTANCE.macroStorage.add(new Macro(name, command, new BindSetting("bind", key)));
                } catch (Exception ignored) {
                }
            }
        }
