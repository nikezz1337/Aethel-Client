...
import antileak.base.client.modules.impl.render.base.implement.*;
...
public class Interface extends Module {
    ...
    private static final boolean PERF_DEBUG = Boolean.parseBoolean(System.getProperty("elysium.perf.debug", "false"));
    private static final long SLOW_HUD_ELEMENT_NANOS = Long.getLong("elysium.perf.hudMs", 5L) * 1_000_000L;
    private static final long PERF_WARN_COOLDOWN_NANOS = Long.getLong("elysium.perf.cooldownMs", 1000L) * 1_000_000L;
    ...
    private final TargetHudtwo TargetHudtwo;
    ...
    private final ModeSetting targetHudStyle = new ModeSetting("Стиль таргет худа", "Первый", "Первый", "Второй");
    ...
    public Interface() {
        ...
        this.waterMark = new WaterMark(elysium.draggable(this, "WaterMark", 10, 10));
        this.keyBinds = new KeyBinds(elysium.draggable(this, "KeyBinds", 30, 30));
        this.helperBinds = new HelperBinds(elysium.draggable(this, "HelperBinds", 90, 30));
        this.potions = new Potions(elysium.draggable(this, "Potions", 30, 60));
        this.cooldowns = new Cooldowns(elysium.draggable(this, "Cooldowns", 30, 90));
        this.staffList = new StaffList(elysium.draggable(this, "StaffList", 60, 100));
        this.information = new Information(elysium.draggable(this, "Information", 50, 100));
        this.notifications = new Notifications(elysium.draggable(this, "Notifications", 0, 0));
        this.targetHud = new TargetHud(elysium.draggable(this, "TargetHud", 30, 90));
        this.TargetHudtwo = new TargetHudtwo(elysium.draggable(this, "TargetHudtwo", 30, 90));
    ...
    private InterfaceProcessing getActiveTargetHud() {
        return targetHudStyle.getCurrent().equals("Р’С‚РѕСЂРѕР№") ? TargetHudtwo : targetHud;
    }
    ...
    private float getMenuHeightForElement(InterfaceProcessing element) {
        if (element == targetHud || element == TargetHudtwo) return 59.0f;
        if (element == waterMark) return 44.0f;
        if (element == information) return 39.0f;
        return 29.0f;
    }
    ...
    public boolean handleHudContextClick(double mouseX, double mouseY, int button) {
        ...
        boolean targetHudContext = hudContextElement == targetHud || hudContextElement == TargetHudtwo;
        float rectButtonY = targetHudContext ? targetHudMenuY + 46.0f
                : (hudContextElement == waterMark ? targetHudMenuY + 30.0f
                   : (informationContext ? targetHudMenuY + 24.0f : targetHudMenuY + 14.0f));
        ...
        if (targetHudContext) {
            TargetHud ctx = hudContextElement instanceof TargetHudtwo ? null : (TargetHud) hudContextElement;
            TargetHudtwo ctx2 = hudContextElement instanceof TargetHudtwo ? (TargetHudtwo) hudContextElement : null;
            ...
            if (ctx != null) {
                ...
                if (button == 0 && normalHovered) { ctx2.setHealthBarStyleEnabled(false); return true; }
                if (button == 0 && unusualHovered) { ctx2.setHealthBarStyleEnabled(true); return true; }
                if (button == 0 && particlesHovered) { ctx2.setHeadParticlesEnabled(!ctx2.isHeadParticlesEnabled()); return true; }
            }
    ...
    public void renderHudContextMenu(DrawContext context, int mouseX, int mouseY) {
        ...
        if (targetHudContext) {
            ...
            boolean healthBarStyle;
            if (hudContextElement instanceof TargetHudtwo ctx2) {
                particlesEnabled = ctx2.isHeadParticlesEnabled();
                healthBarStyle = ctx2.isHealthBarStyleEnabled();
            } else {
                TargetHud ctx = (TargetHud) hudContextElement;
                particlesEnabled = ctx.isHeadParticlesEnabled();
                healthBarStyle = ctx.isHealthBarStyleEnabled();
            }
        ...
        float rectLabelY = targetHudContext ? y + 39.5f : (waterMarkContext ? y + 24.0f : (informationContext ? y + 17.5f : y + 7.5f));
        float rectButtonY = targetHudContext ? y + 46.0f : (waterMarkContext ? y + 30.0f : (informationContext ? y + 24.0f : y + 14.0f));

        issue(12).draw(matrices, "Тип ректа", x + 4.7f, rectLabelY,
                ColorUtils.rgba(255, 255, 255, fadeTextAlphaSafe(contentProgress, 225, 2)));
    ...
    public Map<String, InterfaceProcessing> getConfigurableHudElements() {
        ...
        elements.put("TargetHudtwo", TargetHudtwo);
    ...
    public void onEvent(final EventRender.Default event) {
        ...
        try {
            ...
            if (showTargetHud) {
                if (targetHudStyle.getCurrent().equals("Второй")) {
                    renderHudElement(this.TargetHudtwo, event);
                } else {
