...
import antileak.base.api.events.Priority;
...
public class EntityESP extends Module {
    ...
    private static final float TAG_HUD_RADIUS = 0.5f;
    private static final int TAG_HUD_ALPHA = 150;
    ...
    private final ModeSetting boxType2D = new ModeSetting("Тип 2D боксов", "Углы", "Обычный", "Углы")
            .visible(() -> show2DBoxes.isState());
    ...
    public EntityESP() {
        ...
        addSettings(show3DBox, boxFilled, hurtTint, show2DBoxes, boxType2D, showHealthBar, viewHeldItems, viewEffects, viewEnchants, viewDisplayName);
    }
    ...
    public void onRender2D(EventRender.Default event) {
        ...
        boolean heldItemsEnabled = viewHeldItems.isState();
        boolean effectsEnabled = viewEffects.isState();
        boolean enchantsEnabled = viewEnchants.isState();
        boolean displayNameEnabled = viewDisplayName.isState();
        boolean boxes2DEnabled = show2DBoxes.isState();
        boolean healthBarEnabled = showHealthBar.isState();
        if (!tagsEnabled && !armorEnabled && !heldItemsEnabled && !effectsEnabled && !boxes2DEnabled && !healthBarEnabled) return;

        Font font = (tagsEnabled || heldItemsEnabled || effectsEnabled) ? Fonts.getFont("sf_regular", TAG_FONT_SIZE) : null;
        Font effectFont = (tagsEnabled || heldItemsEnabled || effectsEnabled) ? Fonts.getFont("sf_regular", 13) : null;
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof PlayerEntity player) {
                ...
                if (boxes2DEnabled) {
                    draw2DBox(event, player, rect);
                }
                if (healthBarEnabled) {
                    draw2DHealthBar(event, player, rect);
                }
                ...
                if (heldItemsEnabled && font != null) {
                    drawHeldItems(event, player, rect, displayNameEnabled, font);
                }
                if (effectsEnabled && effectFont != null) {
                    drawEffects(event, player, rect, effectFont);
                }
                continue;
            }
            ...
            if (boxes2DEnabled) {
                draw2DBox(event, livingEntity, rect);
            }
            if (healthBarEnabled && livingEntity instanceof LivingEntity le) {
                draw2DHealthBar(event, le, rect);
            }
    ...
    private void draw2DBox(EventRender.Default event, Entity entity, ScreenRect rect) {
        ...
        int boxColor = isFriend ? 0xFF00FF00 : frameThemeColor;
        int outlineColor = 0xFF1A1A1A;
        float x = rect.minX();
        float y = rect.minY();
        float endX = rect.maxX();
        float endY = rect.maxY();
        boolean cornerMode = boxType2D.getIndex() == 1;
        ...
        if (cornerMode) {
            ...
            float t = 0.6f;
            drawScreenRect(matrices, x - t, y - t, x + (float)sectX, y + t, outlineColor);
            drawScreenRect(matrices, endX - (float)sectX, y - t, endX + t, y + t, outlineColor);
            drawScreenRect(matrices, x - t, endY - t, x + (float)sectX, endY + t, outlineColor);
            drawScreenRect(matrices, endX - (float)sectX, endY - t, endX + t, endY + t, outlineColor);
            drawScreenRect(matrices, x - t, y, x + t, y + (float)sectY, outlineColor);
            drawScreenRect(matrices, x - t, endY - (float)sectY, x + t, endY, outlineColor);
            drawScreenRect(matrices, endX - t, y, endX + t, y + (float)sectY, outlineColor);
            drawScreenRect(matrices, endX - t, endY - (float)sectY, endX + t, endY, outlineColor);
            float inner = 0.25f;
            drawScreenRect(matrices, x - inner, y - inner, x + (float)sectX - inner, y + inner, boxColor);
            drawScreenRect(matrices, endX - (float)sectX + inner, y - inner, endX + inner, y + inner, boxColor);
            drawScreenRect(matrices, x - inner, endY - inner, x + (float)sectX - inner, endY + inner, boxColor);
            drawScreenRect(matrices, endX - (float)sectX + inner, endY - inner, endX + inner, endY + inner, boxColor);
            drawScreenRect(matrices, x - inner, y + inner, x + inner, y + (float)sectY - inner, boxColor);
            drawScreenRect(matrices, x - inner, endY - (float)sectY + inner, x + inner, endY - inner, boxColor);
            drawScreenRect(matrices, endX - inner, y + inner, endX + inner, y + (float)sectY - inner, boxColor);
            drawScreenRect(matrices, endX - inner, endY - (float)sectY + inner, endX + inner, endY - inner, boxColor);
        } else {
            float t = 0.6f;
            drawScreenRect(matrices, x - t, y - t, endX + t, y + t, outlineColor);
            drawScreenRect(matrices, x - t, endY - t, endX + t, endY + t, outlineColor);
            drawScreenRect(matrices, x - t, y, x + t, endY, outlineColor);
            drawScreenRect(matrices, endX - t, y, endX + t, endY, outlineColor);
            float inner = 0.25f;
            drawScreenRect(matrices, x - inner, y - inner, endX + inner, y + inner, boxColor);
            drawScreenRect(matrices, x - inner, endY - inner, endX + inner, endY + inner, boxColor);
            drawScreenRect(matrices, x - inner, y, x + inner, endY, boxColor);
            drawScreenRect(matrices, endX - inner, y, endX + inner, endY, boxColor);
        }
    }
    ...
    private void draw2DHealthBar(EventRender.Default event, LivingEntity entity, ScreenRect rect) {
        ...
        drawScreenRect(matrices, posX - hpOffset - out, posY - out, posX - hpOffset + barWidth + out, posW + out, 0xFF000000);
        drawScreenRect(matrices, posX - hpOffset, posY, posX - hpOffset + barWidth, posW, 0x80000000);
        boolean isFriend = entity instanceof PlayerEntity player
                && elysium.INSTANCE.friendStorage != null
                && elysium.INSTANCE.friendStorage.isFriend(player.getName().getString());
        if (isFriend) {
            drawScreenRect(matrices, posX - hpOffset, barTop, posX - hpOffset + barWidth, posW, 0xFF008006);
        } else {
            drawHealthBarGradient(matrices, posX - hpOffset, barTop, posX - hpOffset + barWidth, posW, hpPercent);
        }
    }
    ...
    private void drawHealthBarGradient(MatrixStack matrices, float left, float top, float right, float bottom, float hpPercent) {
        ...
        if (hpPercent > 0.5f) {
            ...
            int r = (int) (255 * (1 - t));
            color = 0xFF000000 | (r << 16) | (255 << 8);
        } else {
            float t = hpPercent * 2f;
            int g = (int) (255 * t);
            color = 0xFF000000 | (255 << 16) | (g << 8);
        }
        drawScreenRect(matrices, left, top, right, bottom, color);
    }

    private void drawScreenRect(MatrixStack matrices, float x1, float y1, float x2, float y2, int color) {
        RenderUtils.drawRoundedRect(matrices, x1, y1, x2 - x1, y2 - y1,6, color);
    }
    public List<PublicDonateSegment> getDonateSegmentsForStaffList(PlayerEntity player) {
        if (player == null) return Collections.emptyList();
        List<DonateSegment> internal = getDonateSegmentsFromTab(player);
        if (internal.isEmpty()) return Collections.emptyList();
        List<PublicDonateSegment> result = new ArrayList<>(internal.size());
    ...
    private void drawTag(EventRender.Default event, PlayerEntity player, ScreenRect rect, Font font) {
        ...
        String rightBracket = "";
        boolean isFriend = elysium.INSTANCE.friendStorage != null
                && elysium.INSTANCE.friendStorage.isFriend(player.getName().getString());
        ...
        float drawX = x - 7.5f + PLAYER_HEAD_SIZE + PLAYER_HEAD_GAP;
        for (DonateSegment segment : donateSegments) {
    ...
    private void drawHeldItems(EventRender.Default event, LivingEntity entity, ScreenRect rect, boolean displayNameEnabled, Font font) {
        ...
        float y = rect.maxY() + 3.0f;
        if (!mainHand.isEmpty()) {
            y = drawHeldItemLine(matrices, font, rect, y, mainHand, displayNameEnabled);
        }
        if (!offHand.isEmpty()) {
            drawHeldItemLine(matrices, font, rect, y, offHand, displayNameEnabled);
        }
    }

    private float drawHeldItemLine(MatrixStack matrices, Font font, ScreenRect rect, float y, ItemStack stack, boolean displayNameEnabled) {
        String text = buildItemLabel(stack, displayNameEnabled);
        float width = font.getStringWidth(text);
        float x = rect.centerX() - width * 0.5f;
        drawDefaultTagPanel(matrices, x - 3f, y - 0.5f, width + 3.0f, TAG_FONT_SIZE - 3.0f);
        font.drawString(matrices, text, x, y + 3, TAG_TEXT_COLOR);
        return y + TAG_FONT_SIZE;
    }

    private void drawEffects(EventRender.Default event, LivingEntity entity, ScreenRect rect, Font font) {
        ...
        for (StatusEffectInstance effect : effects) {
            ...
            if (amplifier > 1) {
                builder.append(' ').append(toRomanNumeral(amplifier));
            }
            builder.append(' ').append(formatDuration(effect.getDuration()));
            lines.add(new EffectLine(builder.toString(), getEffectColor(effect)));
        }
        ...
        if (lines.size() > MAX_EFFECT_TAGS) {
            lines = lines.subList(0, MAX_EFFECT_TAGS);
        }

        float y = rect.minY() - TAG_FONT_SIZE - 2.0f;
        for (EffectLine line : lines) {
            float width = font.getStringWidth(line.text());
            float x = rect.centerX() - width * 0.5f;
            drawDefaultTagPanel(event.getContext().getMatrices(), x - 1.5f, y - 0.5f, width + 3.0f, TAG_FONT_SIZE - 3.0f);
            font.drawString(event.getContext().getMatrices(), line.text(), x, y + 3, line.color());
            y -= TAG_FONT_SIZE;
        }
    }
    ...
    private String buildItemLabel(ItemStack stack, boolean displayNameEnabled) {
        ...
        String enchants = getEnchantmentsText(stack);
        if (!enchants.isEmpty()) {
            label += " " + enchants;
        }
        if (stack.getCount() > 1) {
    ...
    private String getEnchantmentsText(ItemStack stack) {
        if (!viewEnchants.isState() || stack.isEmpty()) {
            return "";
        }
    ...
    private int getEffectColor(StatusEffectInstance effect) {
        ...
        return color != 0 ? 0xFF000000 | color : TAG_TEXT_COLOR;
    }
    ...
    private void drawDefaultTagPanel(MatrixStack matrices, float x, float y, float width, float height) {
        ...
        RenderUtils.drawRoundedRect(matrices, x, y, width, height, TAG_HUD_RADIUS, ColorUtils.rgba(0, 0, 0, TAG_HUD_ALPHA));
    }
    ...
    private boolean isRainbowTheme() {
        if (elysium.INSTANCE == null || elysium.INSTANCE.themeStorage == null || elysium.INSTANCE.themeStorage.getThemes() == null) {
            return false;
        }
