...
public final class AutoPriceParser implements QuickImports {
    @Getter
    ...
    public static void setEnabled(boolean value) {
        ...
        if (value) {
            ...
            TextUtil.sendMessage("§8[AutoParser] Запущен парсинг " + totalItems + " предметов...");
        } else {
            enabled = false;
            stage = Stage.IDLE;
            currentIndex = 0;
        ...
        switch (stage) {
            case SEND_SEARCH -> {
                if (currentIndex >= itemsToProcess.size()) {
                    ...
                    TextUtil.sendMessage("§8[AutoParser] Все предметы обработаны");
                    return;
                }
                ...
                TextUtil.sendMessage("§7[AutoParser] (" + (currentIndex + 1) + "/" + totalItems + ") Ищем §f" + currentItem.getName() + "§7...");
                mc.player.networkHandler.sendChatCommand("ah search " + searchQuery);
            ...
            case WAIT_RESULTS -> {
                ...
                if (cheapest > 0) {
                    ...
                    TextUtil.sendMessage("§7[AutoParser] §f" + currentItem.getName() + " §a— $" + formatPrice(cheapest) + " → $" + formatPrice(newPrice) + " (скидка " + discountPercent + "%)");
    ...
    private static void disableAndCleanup() {
        ...
        TextUtil.sendMessage("§a[AutoParser] Парсинг завершён! Обработано " + Math.min(currentIndex, totalItems) + "/" + totalItems + " предметов.");
