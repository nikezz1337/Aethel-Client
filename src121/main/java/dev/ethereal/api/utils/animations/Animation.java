package dev.ethereal.api.utils.animations;

import dev.ethereal.api.utils.math.TimerUtils;
...
public class Animation {
    ...
    public void update(boolean forward) {
    	if (this.forward != forward) {
    		this.forward = forward;
        	timer.setStartTime((long) (TimerUtils.now() - (value - Math.min(value, timer.getElapsed()))));
    	}
    }
    ...
    public float getReversedLinear() {
        return 1 - getLinear();
    }
