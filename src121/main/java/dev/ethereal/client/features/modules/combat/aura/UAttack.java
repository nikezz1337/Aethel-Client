...
public class UAttack implements QuickImports {
    ...
    public static long getMsCooldown() {
        ...
        if (Aura.getInstance().attackDelay.is("Динамичный")) {
            msCooldown = ThreadLocalRandom.current().nextLong(470, 520);
    ...
    public static boolean isBestMomentToHit(boolean fallCheck) {
        ...
        if (cpsBypassTrigger() && getYCapacityOnPlayerPos(2) > .20000004768371582D) {
            adaptiveFallValue = .2F;
        }
