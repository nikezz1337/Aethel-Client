...
import dev.ethereal.api.utils.rotation.manager.*;
...
public class AutoBuffModule extends Module {
    ...
    private final MultiBooleanSetting potions = new MultiBooleanSetting("Зелья").value(
            new BooleanSetting("Сила").value(true),
            new BooleanSetting("Скорость").value(true),
            new BooleanSetting("Огнестойкость").value(false)
    );
    private final ModeSetting mode = new ModeSetting("Режим").value("Legit").values("Legit", "Packet");
    private final SliderSetting ticks = new SliderSetting("Тики после спауна").value(10f).range(0f,200f).step(5f);
    private final BooleanSetting autoDisable = new BooleanSetting("Авто отключение").value(false);
    private final BooleanSetting onlyOnGround = new BooleanSetting("Только на земле").value(true);
    ...
    private void rotateDown() {
        ...
        RotationManager.getInstance().addRotation(new RotationChanger(
                100,
                () -> new Float[]{mc.player.getYaw(), 90f},
                () -> --ticks[0] <= 0
        ));
    }
    ...
    private void finish(boolean packet) {
        ...
        RotationManager.getInstance().addRotation(new RotationChanger(
                50,
                () -> new Float[]{mc.player.getYaw(), originalPitch},
                () -> --ticks[0] <= 0
        ));
    ...
    private RegistryEntry<StatusEffect> getEffect(String p) {
        return switch (p) {
            case "Сила" -> StatusEffects.STRENGTH;
            case "Скорость" -> StatusEffects.SPEED;
            case "Огнестойкость" -> StatusEffects.FIRE_RESISTANCE;
            default -> null;
        };
    }
