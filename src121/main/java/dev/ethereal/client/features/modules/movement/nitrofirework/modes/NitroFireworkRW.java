package dev.ethereal.client.features.modules.movement.nitrofirework.modes;
...
import dev.ethereal.client.features.modules.movement.nitrofirework.NitroFireworkMode;
...
public class NitroFireworkRW extends NitroFireworkMode {
    @Override
    public String getName() {
        return "Grim";
    }

    private final ModeSetting mode = new ModeSetting("Grim boost").value("Really World").values("Really World");
    ...
    public NitroFireworkRW(Supplier<Boolean> condition) {
       addSettings(options, speedPlus, distanceBasedSpeed);
    ...
    public Pair<Float, Float> velocityValues() {
        ...
        if (aura.getTarget() != null) {
            Box targetBB = aura.getTarget().getBoundingBox();
            ...
            if (aura.getTarget().isGliding() && elytraTarget.isEnabled()) {
                Vec3d predictedPos = elytraTarget.elytraRotationProcessor.getPredictedPos(aura.getTarget());
                if (predictedPos != null) {
                    bestCandidate = predictedPos;
                }

                if (distanceBasedSpeed.getValue()) {
                    bestCandidate.add(aura.getTarget().getVelocity().multiply(0.5));
                }
            }

            targetVector = bestCandidate;

            if (mc.player.getEyePos().distanceTo(bestCandidate) < 3) {
                inTarget = true;
            }
        }
        ...
        if (distanceBasedSpeed.getValue() && targetVector != null) {
            float distanceFactor = (float) (mc.player.getEyePos().distanceTo(targetVector) / Aura.getInstance().getRange());
