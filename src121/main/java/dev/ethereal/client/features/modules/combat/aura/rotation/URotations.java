...
import dev.ethereal.api.event.events.player.other.MoveInputEvent;
...
import dev.ethereal.client.features.modules.combat.aura.util.GCDUtil;
...
public class URotations extends Component {
    ...
    public static void rotateToCamera() {
        ...
        float yawDelta = MathHelper.wrapDegrees(cameraYaw - mc.player.getYaw());
        float pitchDelta = cameraPitch - mc.player.getPitch();

        mc.player.setYaw(mc.player.getYaw() + GCDUtil.getFixRotate(yawDelta));
        mc.player.setPitch(MathHelper.clamp(
                mc.player.getPitch() + GCDUtil.getFixRotate(pitchDelta), -90F, 90F));
    ...
    public static void update(Rotation target, float yawSpeed, float pitchSpeed,
        ...
        if (rotationState.equals(RotationState.IDLE) && !clientRotation) {
            ...
            if (mc.player != null) movementYaw = mc.player.getYaw();
        }
