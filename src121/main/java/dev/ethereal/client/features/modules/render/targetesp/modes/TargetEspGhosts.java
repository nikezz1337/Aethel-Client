...
public class TargetEspGhosts extends TargetEspMode {
    ...
    private static final int   SPAWN_PER_TICK = 8;
    ...
    // позиция цели на прошлом тике — чтобы заполнять пространственный разрыв при движении
    ...
    public void onUpdate() {
        ...
        double curX = currentTarget.getX();
        double curY = currentTarget.getY();
        double curZ = currentTarget.getZ();
        if (!hasLast) {
            lastX = curX; lastY = curY; lastZ = curZ;
            hasLast = true;
        }

        // Заполняем пространственный разрыв между позицией цели в прошлом и текущем тике.
        // Без этого на скорости точки спавнятся далеко друг от друга и трейл распадается на «капли».
        for (int sub = 0; sub < SPAWN_PER_TICK; sub++) {
            float frac = (sub + 1) / (float) SPAWN_PER_TICK;
            double bx = lastX + (curX - lastX) * frac;
            double by = lastY + (curY - lastY) * frac;
            double bz = lastZ + (curZ - lastZ) * frac;
            long subTime = now - (long)((SPAWN_PER_TICK - 1 - sub) * (50.0 / SPAWN_PER_TICK));
            spawnPoints(subTime, bx, by, bz);
        }
    ...
    private void spawnPoints(long time, double baseX, double baseY, double baseZ) {
        ...
        for (int corner = 0; corner < CORNER_COUNT; corner++) {
            ...
            float px = (float) baseX - (float) Math.sin(yawRad) * xzRange;
            float py = (float) baseY + yRange * yLerpPC + 0.32f;
            float pz = (float) baseZ + (float) Math.cos(yawRad) * xzRange;
