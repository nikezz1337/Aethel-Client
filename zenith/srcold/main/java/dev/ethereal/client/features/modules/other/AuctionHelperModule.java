...
public class AuctionHelperModule extends Module {
    ...
    private final BindSetting keySell = new BindSetting("Sell Key").value(-1);
    ...
    private final SliderSetting sharpnessLevel = new SliderSetting("Sharpness Level").value(7f).range(1f, 7f).step(1f)
    ...
    public void onEvent() {
        ...
        EventListener packetEvent = PacketEvent.getInstance().subscribe(new Listener<>(event -> {
            ...
            if (utilities.isEnabled("Auto Confirm")) {
                if (raw.contains("Введите /ah sell auto confirm") || raw.contains("чтобы подтвердить продажу")) {
                    waitConf = true;
                    confirmTimer.reset();
                }
            }
        ...
        EventListener keyEvent = KeyEvent.getInstance().subscribe(new Listener<>(event -> {
            ...
            if (event.key() == keySell.getValue() && keySell.getValue() != - 1 && mc.currentScreen == null) {
                mc.player.networkHandler.sendChatMessage("/ah sell auto");
                waitConf = true;
                confirmTimer.reset();
            }
        }));

        addEvents(updateEvent, packetEvent, keyEvent);
    }
    ...
    private void handleFastBuy() {
        ...
        if (confirmDelay == 0) {
            confirmDelay = (long) (30L + MathUtil.random(30, 75));
        }

        if (waitConf && confirmTimer.finished(confirmDelay)) {
            mc.player.networkHandler.sendChatMessage("/ah sell auto confirm");
            waitConf = false;
        }
        ...
        if (delay == 0) {
            delay = (long) MathUtil.random(100, 170);
        }

        String title = screen.getTitle().getString();
        if (AuctionUtil.buyMenu(title) && timer.finished(delay) && utilities.isEnabled("Fast Buy")) {
