...
public class TargetMovementPrediction extends Configurable {
    ...
    @Getter private final ModeSetting mode = new ModeSetting("Режим").value(PredictMode.SIMPLE).values(
            PredictMode.values()
    ).onAction(() -> {
        predictMode = Choice.getChoiceByName(getMode().getValue(), PredictMode.values());
    });

    private final BooleanSetting glidingOnly = new BooleanSetting("Только полет").value(true);

    private final SliderSetting multiplier = new SliderSetting("Множитель").value(1.8f).range(0.5f, 6.0f).step(0.1f);
