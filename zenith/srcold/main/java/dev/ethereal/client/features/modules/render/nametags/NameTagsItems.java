...
public class NameTagsItems {
    ...
    public void renderItems(PlayerEntity entity, float x, float y, DrawContext context) {
        ...
        // Без .copy() — не модифицируем стеки
        ...
        float bgX = x - bgW / 2f;
        dev.ethereal.api.utils.render.RenderUtil.BLUR_RECT.draw(matrixStack, bgX, y - bgPad, bgW, bgH, 2f,
            new Color(8, 8, 10, 180));

        boolean enchants = module.options.isEnabled("Зачарования");
    ...
    public void renderSpecialItems(PlayerEntity player, float x, float y, DrawContext context) {
        ...
        for (int i = 0; i < specialItems.size(); i++) {
            ...
            float textWidth = Fonts.MEDIUM.getWidth(itemName, textSize);
            ...
            dev.ethereal.api.utils.render.RenderUtil.BLUR_RECT.draw(matrixStack, bgX, bgY, bgW, bgH, 2f * scale, new Color(20, 20, 30, 200));
            
        Fonts.MEDIUM.drawText(matrixStack, itemName, x - textWidth / 2f, itemY + gap / 1.5f, textSize, new Color(255, 255, 255));
            y += 2f;
        }
    }
