...
public class ItemSwapModule extends Module {
    ...
    private final BooleanSetting lockMovement = new BooleanSetting("Обход Grim").value(true);
    ...
    public ItemSwapModule() {
        addSettings(firstItemSetting, secondItemSetting, bind, swaprender, onlyEnchanted, lockMovement);
    }
        ...
        if (this.bypassActive) {
            if (!this.bypassSwapped && this.bypassSlot != -1) {
                ...
                if (swaprender.getValue() && mc.player != null) {
                         print("AutoSwap - свапнул на " + this.bypassItemName);
                }
