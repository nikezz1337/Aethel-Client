...
public final class AuraUtil implements QuickImports {
    ...
    // отводка — случайная точка вне хитбокса
    ...
    private static void lgScheduleNextDivert() {
        // следующая отводка через 30–150 мс — очень часто
    ...
    public static void onAttack(String mode) {
        switch (mode) {
            ...
            case "Lony Grief":
                attackTimer.reset();
                lgSwitchPoint(true); // при ударе — сразу новый поинт
                break;
            case "FunTime":
                attackTimer.reset();
                lgSwitchPoint(true); // при ударе — новый мультипоинт, без повторов
                break;
            default:
                ++hitCount;
                if (hitCount >= 3.0F) {
    ...
    public static Vec3d getSpookyTimePoint(Entity entity) {
        float safe = 0.06F;
        float horValue = entity.getWidth() / 2.0F * hitCount;
        float verValue = entity.getHeight() / 4.0F * (hitCount + 1.0F);
        Box box = entity.getBoundingBox();
        Vec3d best = getBestVector(entity, 0.0F);
        Vec3d point = new Vec3d(MathHelper.clamp(best.x - (double)horValue, box.minX + (double)safe, box.maxX - (double)safe), MathHelper.clamp(best.y - (double)verValue, box.minY + (double)safe, box.maxY - (double)safe), MathHelper.clamp(best.z + (double)horValue, box.minZ + (double)safe, box.maxZ - (double)safe));
        return point;
    }
