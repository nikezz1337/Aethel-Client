...
public class InventoryUtil implements IMinecraft {
    ...
    public static void useBypass(Item item) {
        if (mc.player == null || mc.getNetworkHandler() == null) return;
        if (mc.player.getItemCooldownManager().isCoolingDown(new ItemStack(item))) return;

        var inventorySlot = searchItem(item, 9, 45);
        var hotbarSlot = searchItem(item, 0, 9);
        var previousSlot = mc.player.getInventory().selectedSlot;

        // Предмет уже в руке — просто юзаем
        if (mc.player.getMainHandStack().getItem() == item) {
            mc.interactionManager.interactItem(mc.player, Hand.MAIN_HAND);
            return;
        }

        // Предмет в оффхенде — просто юзаем
        if (mc.player.getOffHandStack().getItem() == item) {
            mc.interactionManager.interactItem(mc.player, Hand.OFF_HAND);
            return;
        }

        // Останавливаем всё что мешает (как в rockstar перед swap)
        ...
        // === Предмет в хотбаре (как rockstar: selectHotbarSlot → use → restore) ===
        if (hotbarSlot != -1) {
            // 1. Переключаемся на слот с предметом
            mc.player.getInventory().selectedSlot = hotbarSlot;
            mc.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot));

            // 2. Используем предмет
            mc.interactionManager.sendSequencedPacket(mc.world, (sequence) ->
                    new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, sequence, mc.player.getYaw(), mc.player.getPitch()));

            // 3. Восстанавливаем слот
            mc.player.getInventory().selectedSlot = previousSlot;
            mc.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot));
        }

        // === Предмет в инвентаре (как rockstar: hotbarSwap → use → swapBack) ===
        if (hotbarSlot == -1 && inventorySlot != -1) {
            // Используем текущий хотбар слот как временный (как rockstar использует currentSlot)
            var tempSlot = previousSlot;

            // 1. SWAP предмета из инвентаря в хотбар (как InventoryUtility.hotbarSwap)
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, inventorySlot, tempSlot, SlotActionType.SWAP, mc.player);
            NetworkUtils.sendSilentPacket(new CloseHandledScreenC2SPacket(0));

            // 2. Переключаемся на хотбар слот с предметом
            ...
            // 4. SWAP обратно (как rockstar RETURNING_SLOT с hotbarSwap back)
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, inventorySlot, tempSlot, SlotActionType.SWAP, mc.player);
            NetworkUtils.sendSilentPacket(new CloseHandledScreenC2SPacket(0));

            // 5. Восстанавливаем слот
            mc.player.getInventory().selectedSlot = previousSlot;
            mc.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot));
        }
