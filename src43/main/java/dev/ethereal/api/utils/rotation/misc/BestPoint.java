...
@UtilityClass
public class BestPoint implements QuickImports {
    ...
    private boolean isPointVisible(Entity target, Vec3d point, double range) {
        ...
        if (!RaytracingUtil.rayTrace(direction, distance + 0.2, target.getBoundingBox())) {
            return false;
        }

        var blockHit = RaytracingUtil.raycast(eyePos, point, RaycastContext.ShapeType.COLLIDER, mc.player);
    ...
    public static Vec3d getMultipoint(Entity target, double distance) {
        float minMotionXZ = 0.03f;
        float maxMotionXZ = 0.08f;

        float minMotionY = 0.04f;
        float maxMotionY = 0.09f;
        ...
        if (rotationMotion.equals(Vec3d.ZERO))
            rotationMotion = new Vec3d(MathUtil.random(-0.06f, 0.08f), MathUtil.random(-0.05f, 0.05f), MathUtil.random(-0.05f, 0.06f));
        ...
        rotationPoint.add(MathUtil.random(-0.09f, 0.1f), 0f, MathUtil.random(-0.07f, 0.09f));
