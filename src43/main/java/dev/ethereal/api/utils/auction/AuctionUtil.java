...
public class AuctionUtil implements QuickImports {
    ...
    public boolean buyMenu(String name) {
        return name.contains("Покупка предмета") || name.contains("☃") && name.contains("Подтверждение покупки") || name.contains("☃") && name.contains("Подозрительная цена");
    }
    ...
    public boolean isSearchScreen(String title) {
        return title.contains("аук") || title.contains("Маркет") || title.contains("ꈁꀀꈂꌲꈂꀁ") || title.contains("Аукционы") || title.contains("Поиск:") || title.contains("Аукцион") || title.contains("Аукционы ") || title.equals("☃ Аукцион") || title.startsWith("☃ П:") || title.startsWith("☃ Поиск:") || title.startsWith("0A2z");
    }
