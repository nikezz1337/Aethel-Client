package dev.ethereal.api.utils.render.hands;
...
import dev.ethereal.api.utils.render.ShaderPrograms;
...
public class ShaderHandsRenderer implements QuickImports {
    ...
    public void captureBeforeHands() {
        ShaderHandsModule module = ShaderHandsModule.getInstance();
    ...
    public void renderOverlayIfPending() {
        ...
        ShaderProgram maskShader = mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.shaderHandsMaskDiff);
        if (maskShader == null) {
            invalidateState();
            return;
        }
        ...
        RenderSystem.setShader(ShaderPrograms.shaderHandsMaskDiff);
        ...
        float glowValue = module.glow.getValue();
        float fillValue = module.fill.getValue();
        float alphaValue = module.alpha.getValue();
        float outlineValue = module.outline.getValue();

        boolean hasGlow = glowValue > EPSILON;
        boolean hasFill = fillValue > EPSILON && alphaValue > EPSILON;
        Color themeColor = UIColors.primary();
        int color1 = themeColor.getRGB();
        int color2 = color1;
        ...
        ShaderProgram glowShader = hasGlow ? mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.shaderHandsGlow) : null;
        if (glowShader != null) {
            ...
            RenderSystem.setShader(ShaderPrograms.shaderHandsGlow);
            RenderSystem.setShaderTexture(0, blurredMaskTexture);
            RenderSystem.setShaderTexture(1, maskBuffer.getColorAttachment());
            setUniform(glowShader, "color", redf(color1), greenf(color1), bluef(color1));
            setUniform(glowShader, "color2", redf(color2), greenf(color2), bluef(color2));
            setUniform(glowShader, "exposure", 1.0f + glowValue * 1.8f);
            drawFullscreenQuad();
        }

        if (hasFill) {
            ShaderProgram overlayShader = mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.shaderHandsOverlay);
            ...
            RenderSystem.setShader(ShaderPrograms.shaderHandsOverlay);
            RenderSystem.setShaderTexture(0, maskBuffer.getColorAttachment());
            setUniform(overlayShader, "color", redf(color1), greenf(color1), bluef(color1));
    ...
    private int runKawaseBloom(int iterations) {
        ...
        ShaderProgram downShader = mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.shaderHandsKawaseDown);
        ShaderProgram upShader = mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.shaderHandsKawaseUp);
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            RenderSystem.setShader(ShaderPrograms.shaderHandsKawaseDown);
        ...
        for (int i = iterations - 1; i >= 1; i--) {
            ...
            RenderSystem.setShader(ShaderPrograms.shaderHandsKawaseUp);
            RenderSystem.setShaderTexture(0, currentTexture);
    ...
    private void renderPrettyMode(ShaderHandsModule module, int color1, int color2, float glowValue, float fillValue, float alphaValue, float outlineValue) {
        ShaderProgram shader = mc.getShaderLoader().getOrCreateProgram(ShaderPrograms.blockOverlay);
        ...
        RenderSystem.setShader(ShaderPrograms.blockOverlay);
        RenderSystem.setShaderTexture(0, maskBuffer.getColorAttachment());
        ...
        setUniform(shader, "color", redf(color1), greenf(color1), bluef(color1));
        setUniform(shader, "color2", redf(color2), greenf(color2), bluef(color2));
        setUniform(shader, "time", (System.currentTimeMillis() % 100000L) / 1000.0f);
        setUniform(shader, "speed", module.waveSpeed.getValue());
        setUniform(shader, "scale", module.waveScale.getValue());
    ...
    private boolean isEffectEnabled(ShaderHandsModule module) {
        if (module == null || !module.isEnabled()) return false;
        boolean hasGlow = module.glow.getValue() > EPSILON;
        boolean hasFill = module.fill.getValue() > EPSILON && module.alpha.getValue() > EPSILON;
    ...
    private float redf(int color) { return ((color >> 16) & 0xFF) / 255.0f; }
    private float greenf(int color) { return ((color >> 8) & 0xFF) / 255.0f; }
    private float bluef(int color) { return (color & 0xFF) / 255.0f; }
}
