...
public class ColorComponent extends ExpandableComponent.ExpandableSettingComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        float baseHeight = scaled(13f);
        float fontSize = scaled(5.5f);

        int fullAlpha = (int) (getAlpha() * 255f);

        if (setting != null) {
            Fonts.MEDIUM.drawText(ms, setting.getName(), getX() + scaled(1f), getY() + baseHeight / 2f - fontSize / 2f, fontSize, UIColors.textColor(fullAlpha));

            float previewSize = scaled(9f);
            float previewX = getX() + getWidth() - previewSize - scaled(1f);
            float previewY = getY() + baseHeight / 2f - previewSize / 2f;
            float previewRound = scaled(2f);
        ...
        if (animValue > 0.0) {
            ...
            float colorPickerRound = scaled(2f);
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        if (setting != null && MouseUtil.isHovered(mouseX, mouseY, getX(), getY(), getWidth(), scaled(13f))) {
            toggleOpen();
            return;
        }
        
        // Для Theme.ElementColor - клик по квадратику цвета открывает пикер
        if (elementColor != null && MouseUtil.isHovered(mouseX, mouseY, getX(), getY(), getWidth(), scaled(12f))) {
            toggleOpen();
            return;
        }
    ...
    private float getColorPickerY() { return getY() + (setting != null ? scaled(13f) : 0f); }
    ...
    private float getHueHeight() { return scaled(4f) * getAnimValue(); }
