...
public class TargetInfoWidget extends Widget {
    ...
    public void render(Render2DEvent.Render2DEventData event) {
        ...
        if ((PlayerUtil.isFT() || PlayerUtil.isRW()) && target instanceof PlayerEntity player) {
            ScoreboardObjective scoreboard = player.getScoreboard().getObjectiveForSlot(ScoreboardDisplaySlot.BELOW_NAME);
            if (scoreboard != null) {
                ReadableScoreboardScore score = player.getScoreboard().getScore(player, scoreboard);
                MutableText text = ReadableScoreboardScore.getFormattedScore(score, scoreboard.getNumberFormatOr(StyledNumberFormat.EMPTY));
                String parsed = text.getString();
                try {
                    float parsedHp = Float.parseFloat(parsed);
                    if (!parsed.isEmpty() && !parsed.equals("0")) currentHealth = parsedHp;
