...
public class NameTagsItems {
    ...
    private static final String FULL_KRUSH_TEXT = "ФУЛ КРУШ";
    ...
    public void renderItems(PlayerEntity entity, float x, float y, DrawContext context) {
        ...
        float bgX = x - bgW / 2f;
        RenderUtil.RECT.draw(matrixStack, bgX, y - bgPad, bgW, bgH, 2f, new Color(8, 8, 10, 180));
    ...
    public void renderSpecialItems(PlayerEntity player, float x, float y, DrawContext context) {
        ...
        for (int i = 0; i < player.getInventory().size(); i++) {
            ...
            if (isAngelElytra || isTalik) {
                specialLabels.add(new SpecialLabel(itemName, Color.WHITE));
                continue;
            }
        }

        if (hasFullKrushSet) {
            specialLabels.add(0, new SpecialLabel(FULL_KRUSH_TEXT, new Color(255, 70, 70)));
        }
        ...
        for (int i = 0; i < specialLabels.size(); i++) {
            ...
            float textWidth = Fonts.MEDIUM.getWidth(label.text(), textSize);
            ...
            RenderUtil.RECT.draw(matrixStack, bgX, bgY, bgW, bgH, 2f * scale, new Color(20, 20, 30, 200));
            Fonts.MEDIUM.drawText(matrixStack, label.text(), x - textWidth / 2f, itemY + gap / 1.5f, textSize, label.color());
            y += 2f;
        }
    }
    ...
    private void renderEnchantments(ItemStack stack, MatrixStack matrices, float x, float y) {
        ...
        if (stack.hasEnchantments()) {
            ...
            for (Object2IntMap.Entry<RegistryEntry<Enchantment>> entry : enchantments.getEnchantmentEntries()) {
                ...
                Color color = level < max + 1 ? UIColors.textColor() : new Color(255, 80, 80);
    ...
    private record SpecialLabel(String text, Color color) {}
}
