...
public class ThemeManagerWindow implements IMinecraft {
    ...
    public static final File THEME_DIR = new File("Aethel");
    public static final File THEME_FILE = new File(THEME_DIR, "themes.json");
    ...
    public ThemeManagerWindow(ClickGuiFrame parent) {
        ...
        // Дополнительные пресеты из Polyak
        defaultThemes.add(new CustomTheme("Polyak Purple", new Color(170, 68, 255, 255).getRGB(), new Color(13, 13, 16, 255).getRGB(), false));
        defaultThemes.add(new CustomTheme("Ocean Blue", new Color(68, 170, 255, 255).getRGB(), new Color(10, 10, 24, 255).getRGB(), false));
        defaultThemes.add(new CustomTheme("Mint Green", new Color(68, 255, 170, 255).getRGB(), new Color(10, 24, 16, 255).getRGB(), false));
    ...
    public static void initFromFile() {
        ...
        try {
            ...
            if (!json.has("activeTheme")) return;
            ...
            if (json.has("customThemes")) {
                ...
                for (JsonElement el : customArray) {
                    ...
                    if (name.equals(activeName)) {
                        ...
                        dev.aethel.module.settings.impl.ThemeManager.getInstance()
                                .getCurrentTheme().setColors(c1, c2);
                        return;
                    }
                }
            }

            int[] defaultC1 = {0xFFDC143C, 0xFF00C8FF, 0xFFB400FF, 0xFF0066CC, 0xFFC8C8FF, 0xFF64FFE6, 0xFFFFFFFF, 0xFF899FFF, 0xFFAA44FF, 0xFF44AAFF, 0xFF44FFAA};
            int[] defaultC2 = {0xFF1E0000, 0xFF0A0A14, 0xFF1E0028, 0xFF0A0A1E, 0xFF0A0A0F, 0xFF001419, 0xFF000000, 0xFF141423, 0xFF0D0D10, 0xFF0A0A18, 0xFF0A1810};
            String[] defaultNames = {"Crimson", "Cyber Blue", "Violet Void", "Abyss Blue", "Obsidian Glow", "Quantum Shift", "White-Black", "Serenity", "Polyak Purple", "Ocean Blue", "Mint Green"};

            for (int i = 0; i < defaultNames.length; i++) {
                if (defaultNames[i].equals(activeName)) {
                    dev.aethel.module.settings.impl.ThemeManager.getInstance()
                            .getCurrentTheme().setColors(defaultC1[i], defaultC2[i]);
                    return;
                }
            }
        } catch (Exception e) {
    ...
    public List<CustomTheme> getDefaultThemes() {
    ...
    public CustomTheme getActiveTheme() {
        return activeTheme;
    }

    public void setActiveTheme(CustomTheme activeTheme) {
