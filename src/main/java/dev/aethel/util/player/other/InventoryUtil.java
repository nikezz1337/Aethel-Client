package dev.aethel.util.player.other;
...
public class InventoryUtil implements IMinecraft {
    ...
    public static int searchItem(List<Item> items) {
        for (var i = 0; i < mc.player.getInventory().main.size(); i++) {
            for (var item : items) {
                if (mc.player.getInventory().getStack(i).getItem().equals(item)) {
                    return i;
                }
    ...
    public static void swapBypass(Runnable runnable) {
        swapBypass(runnable, 100);
    }

    public static void swapBypass(Runnable runnable, long duration) {
        if (mc.player == null || mc.getNetworkHandler() == null) return;
        try {
    ...
    public static void useDef(Item item) {
        if (mc.player.getItemCooldownManager().isCoolingDown(new ItemStack(item))) return;

        var slot = searchItem(item, 9, mc.player.getInventory().main.size());
    ...
    public static void useItem(Hand hand) {
        ...
        mc.interactionManager.sendSequencedPacket(mc.world, seq ->
                new PlayerInteractItemC2SPacket(hand, seq, yaw, pitch));
        mc.player.swingHand(hand);
    }

    public static void useLegit(Item item) {
        if (mc.player.getItemCooldownManager().isCoolingDown(new ItemStack(item))) return;

        var prevSlot = mc.player.getInventory().selectedSlot;

        // Item already in hand — freeze player + use immediately
        if (mc.player.getMainHandStack().getItem() == item) {
            SlownessManager.addTask(new SlownessManager.SlowTask(100, () -> {}));
        ...
        // Item in hotbar — freeze + swap, use, swap back immediately
        var slotHotbar = searchItem(item, 0, 9);
        if (slotHotbar != -1) {
            SlownessManager.addTask(new SlownessManager.SlowTask(100, () -> {}));
        ...
        // Item in inventory — swap to offhand, use, swap back (all in one bypass)
        var slot = searchItem(item, 9, mc.player.getInventory().main.size());
        if (slot != -1 && slot < mc.player.getInventory().main.size()) {
            swapBypass(() -> {
                clickSlotNoSync(0, slot, 40, SlotActionType.SWAP, mc.player);
                useItem(Hand.OFF_HAND);
                clickSlotNoSync(0, slot, 40, SlotActionType.SWAP, mc.player);
            });
        }
    ...
    public static int findBestElytraSlot() {
        ...
        for (int slot = 0; slot < mc.player.getInventory().main.size(); slot++) {
    ...
    public static int findBestChestplateSlot() {
        ...
        for (var slot = 0; slot < mc.player.getInventory().main.size(); slot++) {
            var stack = mc.player.getInventory().getStack(slot);
            if (stack.getItem() instanceof ArmorItem armor) {
                if (getChestplatePriority(armor) == 0) continue;
    ...
    public static int getBestArmorSlot(EquipmentSlot slot) {
        ...
        for (int i = 0; i < mc.player.getInventory().main.size(); i++) {
            ...
            if (getArmorSlotForItem(armorItem) != slot) continue;
    ...
    private static EquipmentSlot getArmorSlotForItem(Item item) {
    ...
    private static int getChestplatePriority(ArmorItem item) {
