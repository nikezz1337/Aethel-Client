...
public class PlayerIntersectionUtil implements IMinecraft {
    ...
    public void interactItem(Hand hand) {
        ...
        sendSequencedPacket(i -> new PlayerInteractItemC2SPacket(hand, i, rotation.getYaw(), rotation.getPitch()));
    }

    public void interactItem(Hand hand, Rotation rotation) {
        sendSequencedPacket(i -> new PlayerInteractItemC2SPacket(hand, i, rotation.getYaw(), rotation.getPitch()));
    }
    ...
    public void grimSuperBypass(double y, Rotation angle) {
    ...
    public float getHealth(LivingEntity entity) {
        ...
        if (entity instanceof PlayerEntity player) {
            // Server-specific health calculation can be added here
        }
    ...
    public boolean isKey(dev.aethel.module.settings.BindSetting setting) {
        int key = setting.getValue();
        return mc.currentScreen == null && setting.visible.get() && isKey(getKeyType(key), key);
    }
