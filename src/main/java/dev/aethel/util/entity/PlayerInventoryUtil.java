...
import dev.aethel.util.item.ItemUtil;
...
public class PlayerInventoryUtil implements IMinecraft {
    ...
    public void swapAndUse(Slot slot, String text, boolean task) {
        if (slot == null) {
            NotificationRenderer.postWarning(text + " - не найден!");
            return;
        }
        if (task) PlayerInventoryComponent.addTask(() -> swapAndUse(slot, new Rotation(Rotation.cameraYaw(), Rotation.cameraPitch())));
        else swapAndUse(slot, new Rotation(Rotation.cameraYaw(), Rotation.cameraPitch()));
    }

    public void swapAndUse(Item item) {
        swapAndUse(item, new Rotation(Rotation.cameraYaw(), Rotation.cameraPitch()), false);
    }

    public void swapAndUse(Item item, Rotation angle, boolean task) {
        float cooldownProgress = ItemUtil.getCooldownProgress(item);

        if (cooldownProgress > 0) {
            String time = MathUtil.round(cooldownProgress, 0.1) + "с";
            NotificationRenderer.postWarning(item.getName().getString() + " - в кд еще " + time);
            return;
        }

        Slot slot = getSlot(item);
        if (slot == null) {
            NotificationRenderer.postWarning(item.getName().getString() + " - не найден!");
            return;
        }
    ...
    public int getHotbarSlotId(java.util.function.IntPredicate filter) {
