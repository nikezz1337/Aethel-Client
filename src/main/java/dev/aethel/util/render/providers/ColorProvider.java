package dev.aethel.util.render.providers;
...
public final class ColorProvider {
	...
	// Конкретные цвета темы (из Polyak)
	...
	public static int interpolateColor(int from, int to, float amount) {
		...
		int red1 = red(from);
		int green1 = green(from);
		int blue1 = blue(from);
		int alpha1 = alpha(from);
		int red2 = red(to);
		int green2 = green(to);
		int blue2 = blue(to);
		int alpha2 = alpha(to);
	...
	public static int interpolate(int start, int end, float value) {
		...
		return rgba((int) lerp(startColor[0] * 255, endColor[0] * 255, value),
				(int) lerp(startColor[1] * 255, endColor[1] * 255, value),
				(int) lerp(startColor[2] * 255, endColor[2] * 255, value),
				(int) lerp(startColor[3] * 255, endColor[3] * 255, value));
	}
