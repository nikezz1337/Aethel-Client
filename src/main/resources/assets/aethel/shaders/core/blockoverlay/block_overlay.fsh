...
float getCornerFactor(vec2 uv) {
    ...
    float edgeDist = min(min(d.x, d.y), min(1.0 - d.x, 1.0 - d.y));
    float cornerDist = length(d - vec2(max(d.x, d.y)));
    float cornerMask = smoothstep(0.25, 0.0, cornerDist);
    float edgeMask = smoothstep(0.15, 0.05, edgeDist);
    return max(cornerMask, edgeMask * 0.6);
}
...
void main() {
    ...
    float cornerFactor = getCornerFactor(uv);
    ...
    float cornerBoost = cornerFactor * 1.5;
    fillStrength *= (1.0 + cornerBoost);
    edgeStrength *= (1.0 + cornerBoost * 0.8);
